...
class AwNavTestActivity : FragmentActivity() {
    lateinit var nav: AwNav

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(FrameLayout(this).apply { id = TEST_CONTAINER_ID })
        nav = AwNav.init(this, TEST_CONTAINER_ID)
...
class AwNavInstrumentedTest {

    @Test
    fun from_returnsSameInstance_forActivity() {
        val scenario = ActivityScenario.launch(AwNavTestActivity::class.java)

        scenario.onActivity { activity ->
            assertSame(activity.nav, AwNav.from(activity))
        }
